        ...
        for body in dataArray:
            try:
                ...
                if len(newsSummary) > 0:
                    newsSummary0 = str(newsSummary[0].replace(u"\"", "").replace(u"\u2018", "").replace(u"\u2019", "").replace(u"\u201c", "E").replace(u"\xed", "i").replace(u"\xf3", "o").replace(u"\xf1", "n").replace(u"\xe1", "a").encode('utf8'))

                newsTitle = Selector(text=body).xpath("//h3[@class='mod-title']//a/text()").extract() # Link to new
                newsTitle0 = "noTitle"
                if len(newsTitle) > 0:
                    newsTitle0 = str(newsTitle[0].replace(u"\"", "").replace(u"\u2018", "").replace(u"\u2019", "").replace(u"\u201c", "E").replace(u"\xed", "i").replace(u"\xf3", "o").replace(u"\xf1", "n").replace(u"\xe1", "a").encode('utf8'))
