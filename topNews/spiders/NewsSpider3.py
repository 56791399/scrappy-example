...
class NewsSpider3(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        dataArray = response.xpath("//div[@class='term-1']").extract()
        toPrint = ""
        for body in dataArray:
            try:
                ...
                newsSummary = Selector(text=body).xpath("//div[@class='sin-sumario']/text()").extract() # Link to new
                ...
                newsTitle = Selector(text=body).xpath("//div[@class='views-field-title']//a/text()").extract() # Link to new
                newsTitle0 = "noTitle"
                if len(newsTitle) > 0:
